...
export default function Home() {
  ...
  const getBionicReader = async function(type) {
    if (type=="pdf") {
        const pdfFile = new FormData()
        pdfFile.append('file', file)
        axios.post(`${process.env.URL}:4000/speedread`,pdfFile).then(res=> {
            setSpeedReadText(res.data.bionicRead)
        })
        
    } else {
    const data = await fetch(`${process.env.URL}:4000`, {method: "POST",     headers: {
